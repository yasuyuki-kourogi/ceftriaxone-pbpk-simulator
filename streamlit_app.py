...
with st.sidebar:
    st.header("患者パラメータ")
    bw = st.slider("体重 (kg)", 30, 120, 40, 5)
    alb = st.slider("血清アルブミン (g/dL)", 1.0, 5.0, 2.8, 0.1)
    gfr = st.slider("GFR (mL/min)", 5, 150, 45, 5)
    ca_bile = st.number_input("胆汁中 Ca²⁺ (mmol/L)", 1.0, 10.0, 3.0, 0.5)
    gb_conc = st.slider("胆汁濃縮係数", 1, 10, 3, 1)
    ...
    st.markdown("---")
    st.header("投与設計")
    dose_mg = st.selectbox("投与量 (mg)", [500, 1000, 2000], index=1)
