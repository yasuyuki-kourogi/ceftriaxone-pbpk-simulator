...
with st.sidebar:
    st.header("患者パラメータ")
    bw = st.slider("体重 (kg)", 35, 100, 40, 1)
...
with tab4:
    ...
    if sa_type == "体重の影響":
        ...
        st.info(
            ...
            "fT>MICへの影響は単純ではない\n"
            "- **偽胆石リスク**: 本モデルでは胆汁排泄は総血漿中濃度に対する固定クリアランスで実装しているため、ALB低下によるfu上昇は腎排泄増加を介してC_total低下をもたらし、結果として胆汁中濃度は低下する方向に働く。生理学的には遊離型依存の胆汁排泄によりSI上昇が期待されるが、本モデルはその機序を表現しておらず、低ALB条件でのSIは過小評価される可能性がある。"
        )
...
with tab6:
    ...
    with col_m2:
        ...
        st.subheader("制限事項")
        st.error(
            "- Kp値はPK-Sim® v12を用いて算出し、肺・肝臓・腎臓・残余組織で同一値（0.22）であったため本モデルに適用\n"
            "- 本モデルは簡略化PBPKモデルであり、各組織における分布特性を十分に表現できていない可能性がある\n"
