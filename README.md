...
- Kp値: PK-Sim® v12を用いて算出し、肺・肝臓・腎臓・残余組織に同一値（0.22）を適用（Lippert et al. 2019）
...
- **胆汁排泄**: CLbiliary = 0.22 L/h（総血漿中濃度に適用、Alasmari et al. 2023）
- **胆嚢コンパートメント**: 胆汁濃縮・貯留
...
- 食事回数（0–3回/日、時刻は自動設定: 8時/12時/18時）
  - 食事の影響（CCK刺激によるOddi括約筋開放・胆嚢収縮亢進）は食後2時間持続
...
| 🎯 %fT>MIC 解析 | 定常状態の遊離型濃度 vs MIC、投与レジメン比較（8パターン） |
| ⚠️ 偽胆石リスク | Ca-CTRX飽和指数（SI）推移、SI-AUC、超過時間割合、リスク評価 |
| 🔬 感度分析 | 体重・血清アルブミン値・GFR・食事回数による%fT>MIC・SIへの影響（one-at-a-time法） |
...
- Kp値はPK-Sim® v12を用いて算出し、肺・肝臓・腎臓・残余組織で同一値（0.22）を適用したが、簡略化PBPKモデルであるため各組織における分布特性を十分に表現できていない可能性がある
- 胆汁排泄は総血漿中濃度に対する固定クリアランス（CLbiliary = 0.22 L/h）として実装しており、低アルブミン血症による遊離型分率上昇が胆汁排泄速度に反映されない。そのため、低ALB条件ではSIが過小評価される可能性がある
- 胆嚢到達率（空腹時0.5/食後0.0）および排出パラメータは仮定値であり、臨床データによるキャリブレーションは未実施
- Oddi括約筋の開閉は空腹時/食後の二値切替で近似しており、連続的なトーン変動や個体間差は未反映
- 胆嚢内での水分吸収による胆汁濃縮（生理的には5〜20倍）は未反映。実際のSIはモデル予測より高い可能性がある（特に絶食時）
- SIの絶対値は定量的に信頼できないため、条件間の相対比較に用いることが適切
- 腸肝循環は臨床的に無視できるため省略（腸管内で分解）
- 腎排泄は糸球体濾過のみ（尿細管分泌省略）
- 個体間変動（IIV）未実装
- 連続点滴（持続静注）には未対応（Leegwater et al. 2020）
- BP比は条件によらず一定と仮定
...
- Lippert J et al. (2019) *CPT Pharmacometrics Syst Pharmacol* 8:878-882
- Leegwater E et al. (2020) *J Antimicrob Chemother* 75:1554-1558
